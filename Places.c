...
// Last real place must be at index MAX_MAP_LOCATION + NUM_OTHER_LOCATIONS
static Place places[] =
{
   ...
   {"City Unknown", "C?", CITY_UNKNOWN, LAND},
   {"Sea Unknown", "S?", SEA_UNKNOWN, SEA},
   {"Hide", "HI", HIDE, UNKNOWN_LOCATION},
   {"Double Back 1", "D1", DOUBLE_BACK_1, UNKNOWN_LOCATION},
   {"Double Back 2", "D2", DOUBLE_BACK_2, UNKNOWN_LOCATION},
   {"Double Back 3", "D3", DOUBLE_BACK_3, UNKNOWN_LOCATION},
   {"Double Back 4", "D4", DOUBLE_BACK_4, UNKNOWN_LOCATION},
   {"Double Back 5", "D5", DOUBLE_BACK_5, UNKNOWN_LOCATION},
...
{
      if(p >= HIDE && p<= TELEPORT) p -= OTHER_TO_INDEX;
      else if (!validPlace(p)) return "Invalid";
...
{
   if(p >= HIDE && p<= TELEPORT) p -= OTHER_TO_INDEX;
   else if (!validPlace(p)) return UNKNOWN_LOCATION;
...
{
   int lo = MIN_MAP_LOCATION, hi = MAX_MAP_LOCATION + NUM_OTHER_LOCATIONS;
   while (lo <= hi) {
      int mid = (hi+lo)/2;
...
int abbrevToID(char *abbrev) {
   ...
   Place *last = &places[MAX_MAP_LOCATION + NUM_OTHER_LOCATIONS];
   for (p = first; p <= last; p++) {
      char *c = p->abbrev;
      if (c[0] == abbrev[0] && c[1] == abbrev[1] && c[2] == '\0') {
            return p->id;
      }
   }
   return NOWHERE;
...
// given a LocationID, return its abbreviation
char *idToAbbrev(LocationID p)
{
   if(p >= HIDE && p<= TELEPORT) p -= OTHER_TO_INDEX;
   else assert(validPlace(p));
