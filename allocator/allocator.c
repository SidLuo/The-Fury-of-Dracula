...
// Converts offset index to pointers
...
// Converts pointer to offset index
...
void *vlad_malloc(u_int32_t n)
{
	...
	if (curr -> next == ptoi(curr)) {
	   ...
	   case WORST_FIT :
		strategy = WORST_FIT;
		vsize_t worst = -1; // set worst to -1;
		if (curr -> size >= alloc_size) {
                        worst = curr -> size - alloc_size;
                        available = curr;
                }
                curr = (free_header_t *)itop(curr -> next);
                while (ptoi(curr) != free_list_ptr) {
                        ...
                        if (curr -> size >= alloc_size) {
                                // if size can fit and bigger than worst
                                if ((curr -> size - alloc_size) > worst) {
                                        worst = curr -> size - alloc_size;
                                        available = curr;
                                }
                        }
                ...
                if (worst == -1) return NULL; // didn't find a free space
                break;
	   case RANDOM_FIT :
		strategy = RANDOM_FIT;
		if (curr -> size >= alloc_size) available = curr;
                curr = (free_header_t *)itop(curr -> next);
		while (ptoi(curr) != free_list_ptr) {
                        if (curr -> magic != MAGIC_FREE) {
                                fprintf(stderr, "vald_alloc: Memory corruption\n");
                        ...
                        if (curr -> size >= alloc_size) {
                                // if size can fit then break
                                available = curr;
                                break;
                        }
                        curr = (free_header_t *)itop(curr -> next);
                }
                if (ptoi(curr) == free_list_ptr) return NULL; // didn't find a free space
                break;
	   }
	}
	...
	if (available -> size >= threshold) { // split
		...
		alloc_size = available -> size; // allocate the whole free region
...
void vlad_free(void *object)
{
	...
	vlink_t new = ptoi(alloc); // new offset index for header
	if (new < 0) {
	...
	// finding the correct place to insert the region into the list
	...
	// only one free region left and it's after new free region
...
static void vlad_merge()
{
	...
	while (curr -> next != free_list_ptr) {
		...
		if (curr -> next == ptoi(curr) + curr -> size) {
			...
			curr -> size += temp -> size;
			continue; // continue merging based on current free region
		}
		curr = (free_header_t *)itop(curr -> next);
	}
}
...
{
	printf("Strategy: %d, free_list_ptr: %d\n",strategy,free_list_ptr);
	// print out free regions' info
	free_header_t *curr = (free_header_t *) itop(free_list_ptr);
	...
	if (header->magic != MAGIC_ALLOC) header = (alloc_header_t *)(memory + header->size);
	// print out allocated regions' info
	while (ptoi(header)+header->size != ptoi(memory)+memory_size) {
		printf("\tCurrent address: %p\n", header);
		printf("\tOffset index %d, size %d, region %s\n",
			ptoi(header), header->size, (header->magic == MAGIC_ALLOC) ? "ALLOC" : "FREE");
		header = (alloc_header_t *)((byte *)(header) + header->size);
	}
}
